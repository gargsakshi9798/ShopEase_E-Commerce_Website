...
const SuggestedCard = ({ product }) => {
  ...
  return (
    <Link to={`/books/product/${product._id}`} className="bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-md transition-shadow flex flex-col">
      <div className="h-36 bg-gradient-to-br from-yellow-50 to-amber-50 flex items-center justify-center text-6xl relative">
        {product.img}
...
const BooksProductDetail = () => {
  const { id } = useParams();
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const wishlist = useSelector((s) => s.publicWishlist.items);
  const product = booksProducts.find((p) => p._id === id);
  ...
  if (!product) return (
    <div className="min-h-[60vh] flex flex-col items-center justify-center gap-4">
      ...
      <Link to="/books" className="text-primary-600 font-semibold hover:underline">&#8592; Back to Books</Link>
    </div>
  );
  ...
  const related  = booksProducts.filter((p) => p._id !== product._id && p.tag === product.tag).slice(0, 6);
    ...
    <div className="bg-gray-50 min-h-screen">
      <div className="max-w-[1280px] mx-auto px-4 py-3">
        <nav className="flex items-center gap-1.5 text-xs text-gray-500">
          ...
          <Link to="/books" className="hover:text-primary-600">Books</Link><span>/</span>
      ...
      <div className="max-w-[1280px] mx-auto px-4 pb-6">
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="lg:w-[42%] flex-shrink-0">
            <div className="bg-white rounded-2xl border border-gray-100 p-4 sticky top-24">
              <div className="relative h-80 bg-gradient-to-br from-yellow-50 to-amber-50 rounded-xl flex items-center justify-center text-[120px] mb-3 overflow-hidden">
                ...
                <span className="absolute top-3 right-3 text-xs bg-amber-100 text-amber-700 font-bold px-2 py-1 rounded-xl">{disc}% OFF</span>
              ...
              <div className="flex gap-2 overflow-x-auto pb-1">
                {product.images.map((img, i) => (
                  <button key={i} onClick={() => setImgIdx(i)}
                    className={`w-14 h-14 flex-shrink-0 rounded-xl bg-gradient-to-br from-yellow-50 to-amber-50 flex items-center justify-center text-3xl border-2 transition-all ${imgIdx === i ? "border-primary-500 shadow-sm" : "border-transparent hover:border-gray-300"}`}>
                    {img}
                  </button>
                ))}
              </div>
          ...
          <div className="flex-1 min-w-0 space-y-4">
            ...
            {product.sizes.length > 1 && product.sizes[0] !== "One Size" && (
              <div className="bg-white rounded-2xl border border-gray-100 p-5">
                <p className="text-sm font-bold text-gray-800 mb-3">Format: <span className="text-primary-600">{selectedSize}</span></p>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-5">
              ...
              <div className="mt-4 grid grid-cols-3 gap-3">
                {[
                  ...
                  { icon: <MdLoop size={20} className="text-green-600" />,            label: "Easy Returns",   sub: "Hassle-free" },
                  { icon: <MdSecurity size={20} className="text-blue-600" />,         label: "100% Genuine",   sub: "Authentic" },
                ].map((item) => (
        ...
        {related.length > 0 && (
          <div className="mt-8">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-bold text-gray-900">&#128218; More Books You'll Love</h2>
              <Link to="/books" className="text-sm text-primary-600 font-semibold hover:underline">View All</Link>
            </div>
        ...
        {topPicks.length > 0 && (
          <div className="mt-8">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-bold text-gray-900">&#11088; Bestsellers &amp; Top Rated</h2>
              <Link to="/books" className="text-sm text-primary-600 font-semibold hover:underline">View All</Link>
            </div>
...
export default BooksProductDetail;
