...
import { beautyProducts, discount, badgeColor } from "../../data/beautyData";

const subcategories = [
  { icon: "💄", label: "Makeup",      tab: "makeup",      count: 1240 },
  { icon: "🧴", label: "Skincare",    tab: "skincare",    count: 980  },
  { icon: "🛁", label: "Bath & Body", tab: "bath_body",   count: 540  },
  { icon: "🧖", label: "Hair Care",   tab: "hair_care",   count: 760  },
  { icon: "💅", label: "Nail Care",   tab: "makeup",      count: 320  },
  { icon: "🌸", label: "Fragrance",   tab: "all",         count: 290  },
  { icon: "🪒", label: "Men's Care",  tab: "all",         count: 410  },
  { icon: "🌿", label: "Natural",     tab: "skincare",    count: 630  },
];

const brands = ["Lakme", "Maybelline", "Neutrogena", "L'Oreal Paris", "Mamaearth", "Biotique", "Dove", "NYX Professional", "The Ordinary", "Himalaya", "Colorbar", "WOW Skin Science"];
...
const ProductCard = ({ product, view }) => {
  ...
  if (view === "list") {
    return (
      <Link to={`/beauty/product/${product._id}`}
        className="bg-white rounded-2xl shadow-sm border border-gray-100 flex gap-4 p-4 hover:shadow-md transition-shadow group">
        <div className="w-32 h-32 bg-gradient-to-br from-pink-50 to-rose-50 rounded-xl flex items-center justify-center text-6xl flex-shrink-0 relative">
          {product.img}
          ...
          {product.colors.length > 1 && (
            <div className="flex gap-1 mt-1.5">
              {product.colors.slice(0, 5).map((c) => (
                <span key={c.name} title={c.name} className="w-4 h-4 rounded-full border border-gray-200 flex-shrink-0" style={{ backgroundColor: c.hex }} />
              ))}
              {product.colors.length > 5 && <span className="text-[10px] text-gray-400 ml-1">+{product.colors.length - 5}</span>}
            </div>
          )}
  ...
  return (
    <Link to={`/beauty/product/${product._id}`}
      ...
      <div className="relative bg-gradient-to-br from-pink-50 to-rose-50 h-44 flex items-center justify-center text-7xl">
        {product.img}
      ...
      <div className="p-3 flex flex-col flex-1">
        ...
        {product.colors.length > 1 && (
          <div className="flex gap-1 mt-1.5">
            {product.colors.slice(0, 5).map((c) => (
              <span key={c.name} title={c.name} className="w-3.5 h-3.5 rounded-full border border-gray-200" style={{ backgroundColor: c.hex }} />
            ))}
          </div>
        )}
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-4 sticky top-24">
      ...
      <Section title="Price Range" id="price">
        <div className="px-1">
          <input type="range" min={0} max={2000} step={100} value={priceRange[1]} onChange={(e) => setPriceRange([0, +e.target.value])} className="w-full accent-primary-600" />
...
const Beauty = () => {
  ...
  const filtered = activeTab === "all" ? beautyProducts : beautyProducts.filter((p) => p.tag === activeTab);
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      {/* Hero */}
      <div className="bg-gradient-to-r from-[#4a0032] via-[#831843] to-[#be185d] text-white">
        <div className="max-w-[1280px] mx-auto px-4 py-10 flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            <div className="flex items-center gap-2 mb-3">
              <FaSpa className="text-pink-300" size={18} />
              <span className="text-pink-300 font-bold text-sm tracking-wide uppercase">Beauty Bonanza Sale</span>
            </div>
            <h1 className="text-4xl md:text-5xl font-extrabold leading-tight mb-3">
              Glow Up with <span className="text-pink-300">Premium</span> Beauty
            </h1>
            <p className="text-white/70 max-w-md text-sm leading-relaxed mb-5">
              Shop skincare, makeup, hair care and wellness from India's most loved beauty brands. Up to 60% off + free gifts on orders above &#8377;999.
            </p>
            <div className="flex flex-wrap gap-3 mb-5">
              {["100% Authentic", "Free Samples", "Cruelty-Free Options", "Easy Returns"].map((f) => (
            ...
            <div className="flex items-center gap-3">
              <Link to="/" className="inline-flex items-center gap-2 bg-pink-400 hover:bg-pink-500 text-white font-bold px-6 py-3 rounded-xl text-sm transition-colors shadow-lg">
              ...
              <div className="flex items-center gap-2 bg-white/10 backdrop-blur px-4 py-3 rounded-xl border border-white/20">
                ...
                <span className="text-sm font-bold">Use <span className="text-yellow-300">BEAUTY20</span> &#8594; 20% off</span>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-4 sm:grid-cols-8 gap-3">
          {subcategories.map((s) => (
            <button key={s.label} onClick={() => setActiveTab(s.tab)}
              className="bg-white rounded-2xl p-3 flex flex-col items-center gap-2 shadow-sm border border-gray-100 hover:border-pink-300 hover:shadow-md transition-all group">
              <span className="text-3xl">{s.icon}</span>
              <span className="text-[11px] font-semibold text-gray-700 group-hover:text-pink-600 text-center leading-tight">{s.label}</span>
      ...
      <div className="max-w-[1280px] mx-auto px-4 pb-4">
        <div className="bg-gradient-to-r from-pink-50 to-rose-50 border border-pink-100 rounded-2xl p-4 flex flex-wrap items-center gap-4">
          <div className="flex items-center gap-2">
            <span className="text-2xl">🌸</span>
            <div>
              <p className="text-sm font-bold text-gray-900">Today's Beauty Deals</p>
              <p className="text-xs text-gray-500">Free gift on orders above &#8377;999</p>
            </div>
          </div>
          {[{ label: "Face Wash from", price: "&#8377;149" }, { label: "Lipstick from", price: "&#8377;299" }, { label: "Serums from", price: "&#8377;549" }, { label: "Hair Oil from", price: "&#8377;399" }].map((d) => (
            <div key={d.label} className="bg-white rounded-xl px-3 py-2 border border-pink-100 text-center shadow-sm">
              ...
              <p className="text-sm font-bold text-pink-600" dangerouslySetInnerHTML={{ __html: d.price }} />
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="bg-white rounded-2xl border border-gray-100 px-4 py-3 flex flex-wrap items-center gap-3 mb-4 shadow-sm">
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              <button onClick={() => setMobileFilter(true)} className="lg:hidden flex items-center gap-1.5 border border-gray-200 rounded-lg px-3 py-1.5 text-xs font-medium text-gray-600 hover:bg-gray-50">
