...
import {
  ...
  MdBatteryChargingFull, MdCamera, MdMemory,
} from "react-icons/md";
...
const subcategories = [
  { icon: "🏆", label: "Flagship",    tab: "flagship", count: 380  },
  { icon: "⚡", label: "Mid-Range",   tab: "midrange", count: 640  },
  { icon: "💰", label: "Budget",      tab: "budget",   count: 920  },
  { icon: "📸", label: "Best Camera", tab: "flagship", count: 210  },
  { icon: "🔋", label: "Big Battery", tab: "midrange", count: 310  },
  { icon: "🎮", label: "Gaming",      tab: "budget",   count: 180  },
  { icon: "📶", label: "5G Phones",   tab: "all",      count: 1240 },
  { icon: "🔥", label: "New Arrivals",tab: "all",      count: 95   },
];

const brands = ["Apple", "Samsung", "OnePlus", "Xiaomi", "Google", "Realme", "Motorola", "Vivo", "OPPO", "Poco", "Nothing"];
...
const ProductCard = ({ product, view }) => {
  ...
  if (view === "list") {
    return (
      <Link to={`/mobiles/product/${product._id}`}
        ...
        <div className="w-32 h-32 bg-gradient-to-br from-indigo-50 to-purple-50 rounded-xl flex items-center justify-center text-6xl flex-shrink-0 relative">
          {product.img}
          {product.badge && (
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 mt-1.5 flex-wrap">
            <span className="flex items-center gap-1 text-[11px] text-gray-500">
              <MdMemory size={12} className="text-indigo-400" /> {product.ram} RAM
            </span>
            <span className="flex items-center gap-1 text-[11px] text-gray-500">
              <MdCamera size={12} className="text-purple-400" /> {product.camera}
            </span>
            <span className="flex items-center gap-1 text-[11px] text-gray-500">
              <MdBatteryChargingFull size={12} className="text-green-500" /> {product.battery}
            </span>
          </div>
        ...
        <div className="flex flex-col gap-2 justify-center flex-shrink-0">
          <button onClick={handleWishlist}
            className="w-9 h-9 rounded-full border border-gray-200 flex items-center justify-center hover:border-rose-400 transition-colors">
            {isWished ? <MdFavorite size={18} className="text-rose-500" /> : <MdFavoriteBorder size={18} className="text-gray-400" />}
          </button>
          <button onClick={handleAddToCart}
            className="flex items-center gap-1.5 bg-primary-600 hover:bg-primary-700 text-white text-xs font-bold px-3 py-2 rounded-xl transition-colors">
  ...
  return (
    <Link to={`/mobiles/product/${product._id}`}
      ...
      <div className="relative bg-gradient-to-br from-indigo-50 to-purple-50 h-44 flex items-center justify-center text-7xl">
        {product.img}
        {product.badge && (
      ...
      <div className="p-3 flex flex-col flex-1">
        ...
        <div className="flex flex-wrap gap-1 mt-1.5">
          <span className="text-[10px] bg-indigo-50 text-indigo-600 font-semibold px-1.5 py-0.5 rounded-md">{product.ram}</span>
          <span className="text-[10px] bg-purple-50 text-purple-600 font-semibold px-1.5 py-0.5 rounded-md">{product.camera}</span>
          <span className="text-[10px] bg-green-50 text-green-600 font-semibold px-1.5 py-0.5 rounded-md">{product.battery}</span>
        </div>
        <div className="flex items-center gap-2 mt-2">
    ...
    <div className="bg-white rounded-2xl border border-gray-100 p-4 sticky top-24">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-2">
          <button onClick={() => setFilters({ brands: [] })} className="text-xs text-primary-600 font-semibold hover:underline">
            Clear All
          </button>
...
const Mobiles = () => {
  ...
  const filtered = activeTab === "all"
    ? mobilesProducts
    : mobilesProducts.filter((p) => p.tag === activeTab);
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
        ...
        <div className="max-w-[1280px] mx-auto px-4 py-10 flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            <div className="flex items-center gap-2 mb-3">
              <FaMobileAlt className="text-violet-400" size={18} />
              <span className="text-violet-400 font-bold text-sm tracking-wide uppercase">Mobile Fest Sale</span>
            </div>
            <h1 className="text-4xl md:text-5xl font-extrabold leading-tight mb-3">
              Find Your <span className="text-violet-400">Perfect</span><br />Smartphone
            </h1>
            <p className="text-white/70 max-w-md text-sm leading-relaxed mb-5">
              From budget-friendly picks to flagship powerhouses — explore 5G phones from Apple,
              Samsung, OnePlus and more. Up to 40% off on top brands.
            </p>
            <div className="flex flex-wrap items-center gap-3 mb-5">
              {["5G Ready", "EMI Available", "Free Delivery", "1-Year Warranty"].map((f) => (
                <span key={f}
                  className="flex items-center gap-1.5 bg-white/10 border border-white/20 rounded-lg px-3 py-1.5 text-xs font-semibold">
                  &#10003; {f}
                </span>
              ))}
            </div>
            <div className="flex items-center gap-3">
              <Link to="/electronics"
                className="inline-flex items-center gap-2 bg-violet-500 hover:bg-violet-600 text-white font-bold px-6 py-3 rounded-xl text-sm transition-colors shadow-lg">
              ...
              <div className="flex items-center gap-2 bg-white/10 backdrop-blur px-4 py-3 rounded-xl border border-white/20">
                ...
                <span className="text-sm font-bold">Use <span className="text-yellow-400">MOBILE20</span> &#8594; Extra 20% off</span>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-4 sm:grid-cols-8 gap-3">
          {subcategories.map((s) => (
            <button key={s.label} onClick={() => setActiveTab(s.tab)}
              className="bg-white rounded-2xl p-3 flex flex-col items-center gap-2 shadow-sm border border-gray-100 hover:border-violet-300 hover:shadow-md transition-all group">
              <span className="text-3xl">{s.icon}</span>
              <span className="text-[11px] font-semibold text-gray-700 group-hover:text-violet-600 text-center leading-tight">{s.label}</span>
      ...
      <div className="max-w-[1280px] mx-auto px-4 pb-4">
        <div className="bg-gradient-to-r from-violet-50 to-indigo-50 border border-violet-100 rounded-2xl p-4 flex flex-wrap items-center gap-4">
          <div className="flex items-center gap-2">
            <span className="text-2xl">📱</span>
            <div>
              <p className="text-sm font-bold text-gray-900">Today's Mobile Deals</p>
              <p className="text-xs text-gray-500">Limited time offers on top smartphone brands</p>
            </div>
          </div>
          {[
            { label: "Budget Phones from",  price: "&#8377;9,999"   },
            { label: "5G Phones from",      price: "&#8377;14,999"  },
            { label: "Mid-Range from",      price: "&#8377;24,999"  },
            { label: "Flagships from",      price: "&#8377;64,999"  },
          ].map((d) => (
            <div key={d.label}
              className="bg-white rounded-xl px-3 py-2 border border-violet-100 text-center shadow-sm">
              <p className="text-[10px] text-gray-500">{d.label}</p>
              <p className="text-sm font-bold text-violet-600" dangerouslySetInnerHTML={{ __html: d.price }} />
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="max-w-[1280px] mx-auto px-4 pb-10 flex gap-6">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-500 mb-3">
            ...
            {activeTab !== "all" && (
              <> in <span className="font-semibold text-primary-600">{tabs.find((t) => t.key === activeTab)?.label}</span></>
            )}
          </p>
