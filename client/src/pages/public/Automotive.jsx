...
const subcategories = [
  { icon: "🛞", label: "Tyres",         tab: "tyres",       count: 420 },
  { icon: "🔋", label: "Batteries",     tab: "batteries",   count: 280 },
  { icon: "✨", label: "Car Care",      tab: "care",        count: 540 },
  { icon: "⛑️", label: "Helmets",       tab: "helmets",     count: 390 },
  { icon: "💡", label: "Lighting",      tab: "lighting",    count: 310 },
  { icon: "🗺️", label: "GPS & Dash Cam",tab: "electronics", count: 180 },
  { icon: "🛢️", label: "Oils & Fluids", tab: "fluids",      count: 260 },
  { icon: "📢", label: "Accessories",   tab: "accessories", count: 680 },
];

const brands = ["Bosch", "3M", "Michelin", "Vega", "Amaron", "Meguiar's", "Garmin", "Philips", "Viedome", "Hella", "Castrol", "CEAT"];
...
const ProductCard = ({ product, view }) => {
  ...
  if (view === "list") {
    return (
      <Link to={`/automotive/product/${product._id}`}
        ...
        <div className="w-32 h-32 bg-gradient-to-br from-slate-50 to-zinc-50 rounded-xl flex items-center justify-center text-6xl flex-shrink-0 relative">
          {product.img}
  ...
  return (
    <Link to={`/automotive/product/${product._id}`}
      ...
      <div className="relative bg-gradient-to-br from-slate-50 to-zinc-50 h-44 flex items-center justify-center text-7xl">
        {product.img}
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-4 sticky top-24">
      ...
      <Section title="Price Range" id="price">
        <div className="px-1">
          <input type="range" min={0} max={15000} step={500} value={priceRange[1]}
...
const Automotive = () => {
  ...
  const filtered = activeTab === "all" ? automotiveProducts : automotiveProducts.filter((p) => p.tag === activeTab);
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      {/* Hero Banner */}
      <div className="bg-gradient-to-r from-[#0c0c0c] via-[#1c1917] to-[#292524] text-white">
        <div className="max-w-[1280px] mx-auto px-4 py-10 flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            <div className="flex items-center gap-2 mb-3">
              <FaCar className="text-red-400" size={18} />
              <span className="text-red-400 font-bold text-sm tracking-wide uppercase">Automotive Mega Sale</span>
            </div>
            <h1 className="text-4xl md:text-5xl font-extrabold leading-tight mb-3">
              Everything Your <span className="text-red-400">Car Needs</span>
            </h1>
            <p className="text-white/70 max-w-md text-sm leading-relaxed mb-5">
              Tyres, batteries, car care, helmets, GPS devices and premium accessories from top automotive brands. Up to 45% off with free delivery.
            </p>
            <div className="flex flex-wrap gap-3 mb-5">
              {["Free Delivery", "Expert Support", "Genuine Parts", "EMI Available"].map((f) => (
            ...
            <div className="flex items-center gap-3">
              <Link to="/" className="inline-flex items-center gap-2 bg-red-500 hover:bg-red-600 text-white font-bold px-6 py-3 rounded-xl text-sm transition-colors shadow-lg">
              ...
              <div className="flex items-center gap-2 bg-white/10 backdrop-blur px-4 py-3 rounded-xl border border-white/20">
                ...
                <span className="text-sm font-bold">Use <span className="text-yellow-300">AUTO10</span> &#8594; Extra 10% off</span>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-4 sm:grid-cols-8 gap-3">
          {subcategories.map((s) => (
            <button key={s.label} onClick={() => setActiveTab(s.tab)}
              className="bg-white rounded-2xl p-3 flex flex-col items-center gap-2 shadow-sm border border-gray-100 hover:border-red-300 hover:shadow-md transition-all group">
              <span className="text-3xl">{s.icon}</span>
              <span className="text-[11px] font-semibold text-gray-700 group-hover:text-red-700 text-center leading-tight">{s.label}</span>
      ...
      <div className="max-w-[1280px] mx-auto px-4 pb-4">
        <div className="bg-gradient-to-r from-slate-50 to-zinc-50 border border-slate-200 rounded-2xl p-4 flex flex-wrap items-center gap-4">
          <div className="flex items-center gap-2">
            <span className="text-2xl">🚗</span>
            <div>
              <p className="text-sm font-bold text-gray-900">Today's Automotive Deals</p>
              <p className="text-xs text-gray-500">Limited offers on top auto brands</p>
            </div>
          </div>
          {[
            { label: "Car Care from",  price: "&#8377;549"   },
            { label: "Helmets from",   price: "&#8377;1,299" },
            { label: "Batteries from", price: "&#8377;4,999" },
            { label: "Tyres from",     price: "&#8377;3,999" },
          ].map((d) => (
            <div key={d.label} className="bg-white rounded-xl px-3 py-2 border border-slate-200 text-center shadow-sm">
              <p className="text-[10px] text-gray-500">{d.label}</p>
              <p className="text-sm font-bold text-slate-700" dangerouslySetInnerHTML={{ __html: d.price }} />
            </div>
          ))}
        </div>
      </div>
