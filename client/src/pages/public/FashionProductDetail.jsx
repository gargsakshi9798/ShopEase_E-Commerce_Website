...
// ─── Small Product Card (suggestions) ────────────────────────────────────────
const SuggestedCard = ({ product }) => {
  ...
  return (
    <Link to={`/fashion/product/${product._id}`}
      ...
      <div className="h-36 bg-gray-50 flex items-center justify-center text-6xl relative">
        {product.img}
        {product.badge && (
...
const FashionProductDetail = () => {
  ...
  const product = fashionProducts.find((p) => p._id === id);
  ...
  if (!product) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center gap-4">
        ...
        <Link to="/fashion" className="text-primary-600 font-semibold hover:underline">← Back to Fashion</Link>
      </div>
    );
  }
  ...
  // Suggestions: different tag, lower price or better quality
  const cheaper   = fashionProducts.filter((p) => p._id !== product._id && p.price <= product.price * 0.9).slice(0, 6);
  const quality   = fashionProducts.filter((p) => p._id !== product._id && p.rating >= 4.4).slice(0, 6);

  const handleAddToCart = () => {
    if (product.sizes[0] !== "One Size" && !selectedSize) {
      toast.error("Please select a size"); return;
    }
    dispatch(addToCart({ product: { ...product, selectedSize, selectedColor }, qty }));
    ...
    <div className="bg-gray-50 min-h-screen">
      ...
      <div className="max-w-[1280px] mx-auto px-4 py-3">
        <nav className="flex items-center gap-1.5 text-xs text-gray-500">
          ...
          <span>/</span>
          <Link to="/fashion" className="hover:text-primary-600">Fashion</Link>
          <span>/</span>
      ...
      <div className="max-w-[1280px] mx-auto px-4 pb-6">
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="lg:w-[42%] flex-shrink-0">
            <div className="bg-white rounded-2xl border border-gray-100 p-4 sticky top-24">
              {/* Main image */}
              <div className="relative h-80 bg-gray-50 rounded-xl flex items-center justify-center text-[120px] mb-3 overflow-hidden">
              ...
              <div className="flex gap-2 overflow-x-auto pb-1">
                {product.images.map((img, i) => (
                  <button key={i} onClick={() => setImgIdx(i)}
                    className={`w-14 h-14 flex-shrink-0 rounded-xl bg-gray-50 flex items-center justify-center text-3xl border-2 transition-all ${
            ...
            {product.sizes[0] !== "One Size" && (
              <div className="bg-white rounded-2xl border border-gray-100 p-5">
                <p className="text-sm font-bold text-gray-800 mb-3">
                  ...
                  {!selectedSize && <span className="text-red-400 text-xs ml-1">(Select a size)</span>}
                </p>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-5">
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <div className="flex items-center border border-gray-200 rounded-xl overflow-hidden">
                  <button onClick={() => setQty(q => Math.max(1, q-1))}
                    className="w-9 h-9 flex items-center justify-center text-gray-600 hover:bg-gray-100 transition-colors font-bold text-lg">−</button>
              ...
              <div className="mt-4 grid grid-cols-3 gap-3">
                ...
                <div className="flex flex-col items-center text-center gap-1">
                  ...
                  <p className="text-[11px] font-semibold text-gray-700">Secure Payment</p>
        ...
        {/* ── Tabs: Description / Highlights / Specs ── */}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {Object.entries(product.specifications).map(([k, v]) => (
                  <div key={k} className="flex items-start gap-3 bg-gray-50 rounded-xl px-4 py-2.5">
                    <span className="text-xs font-bold text-gray-500 w-32 flex-shrink-0">{k}</span>
        ...
        {/* ── Suggested: Budget Picks ── */}
        {cheaper.length > 0 && (
          <div className="mt-8">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-bold text-gray-900">💸 Budget-Friendly Picks</h2>
              <Link to="/fashion" className="text-sm text-primary-600 font-semibold hover:underline">View All</Link>
            </div>
        ...
        {/* ── Suggested: Excellent Quality ── */}
        {quality.length > 0 && (
          <div className="mt-8">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-bold text-gray-900">⭐ Excellent Quality Products</h2>
              <Link to="/fashion" className="text-sm text-primary-600 font-semibold hover:underline">View All</Link>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
              {quality.map(p => <SuggestedCard key={p._id} product={p} />)}
            </div>
          </div>
        )}
      </div>
