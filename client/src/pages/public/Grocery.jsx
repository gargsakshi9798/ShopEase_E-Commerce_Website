...
import { groceryProducts, discount, badgeColor } from "../../data/groceryData";

const subcategories = [
  { icon: "🌾", label: "Staples",      tab: "staples",      count: 840 },
  { icon: "🥛", label: "Dairy",        tab: "dairy",        count: 420 },
  { icon: "🫙", label: "Oils & Ghee",  tab: "oils",         count: 310 },
  { icon: "☕", label: "Beverages",    tab: "beverages",    count: 580 },
  { icon: "🍜", label: "Instant Food", tab: "instant_food", count: 340 },
  { icon: "🌶️", label: "Spices",       tab: "spices",       count: 460 },
  { icon: "🥜", label: "Snacks",       tab: "snacks",       count: 720 },
  { icon: "🌿", label: "Organic",      tab: "staples",      count: 280 },
];

const brands = ["Tata", "Amul", "India Gate", "Saffola", "Nescafe", "Maggi", "Haldiram's", "MDH", "Fortune", "Patanjali", "Epigamia", "Organic India"];
...
const ProductCard = ({ product, view }) => {
  ...
  if (view === "list") {
    return (
      <Link to={`/grocery/product/${product._id}`}
        className="bg-white rounded-2xl shadow-sm border border-gray-100 flex gap-4 p-4 hover:shadow-md transition-shadow group">
        <div className="w-32 h-32 bg-gradient-to-br from-green-50 to-lime-50 rounded-xl flex items-center justify-center text-6xl flex-shrink-0 relative">
          {product.img}
  ...
  return (
    <Link to={`/grocery/product/${product._id}`}
      ...
      <div className="relative bg-gradient-to-br from-green-50 to-lime-50 h-44 flex items-center justify-center text-7xl">
        {product.img}
...
const Grocery = () => {
  ...
  const filtered = activeTab === "all" ? groceryProducts : groceryProducts.filter((p) => p.tag === activeTab);
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      {/* Hero Banner */}
      <div className="bg-gradient-to-r from-[#052e16] via-[#14532d] to-[#15803d] text-white">
        <div className="max-w-[1280px] mx-auto px-4 py-10 flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            <div className="flex items-center gap-2 mb-3">
              <FaLeaf className="text-lime-300" size={18} />
              <span className="text-lime-300 font-bold text-sm tracking-wide uppercase">Fresh Grocery Sale</span>
            </div>
            <h1 className="text-4xl md:text-5xl font-extrabold leading-tight mb-3">
              Fresh Grocery <span className="text-lime-300">Delivered Fast</span>
            </h1>
            <p className="text-white/70 max-w-md text-sm leading-relaxed mb-5">
              Stock up on staples, dairy, beverages, spices and snacks from India's most trusted grocery brands. Up to 40% off with free next-day delivery.
            </p>
            <div className="flex flex-wrap gap-3 mb-5">
              {["Next-Day Delivery", "FSSAI Certified", "100% Genuine", "Easy Returns"].map((f) => (
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2 bg-white/10 backdrop-blur px-4 py-3 rounded-xl border border-white/20">
                ...
                <span className="text-sm font-bold">Use <span className="text-yellow-300">GROCERY10</span> &#8594; 10% off</span>
              </div>
            </div>
          </div>
      ...
      <div className="max-w-[1280px] mx-auto px-4 pb-4">
        <div className="bg-gradient-to-r from-green-50 to-lime-50 border border-green-100 rounded-2xl p-4 flex flex-wrap items-center gap-4">
          <div className="flex items-center gap-2">
            <span className="text-2xl">🛒</span>
            <div>
              <p className="text-sm font-bold text-gray-900">Today's Grocery Deals</p>
              <p className="text-xs text-gray-500">Fresh savings on everyday essentials</p>
            </div>
          </div>
          {[
            { label: "Staples from",   price: "&#8377;89"  },
            { label: "Dairy from",     price: "&#8377;149" },
            { label: "Beverages from", price: "&#8377;199" },
            { label: "Snacks from",    price: "&#8377;99"  },
          ].map((d) => (
