...
import {
  ...
  MdShoppingCart, MdLocalShipping, MdVerified, MdLoop,
...
const SuggestedCard = ({ product }) => {
  ...
  return (
    <Link to={`/beauty/product/${product._id}`}
      ...
      <div className="h-36 bg-gradient-to-br from-pink-50 to-rose-50 flex items-center justify-center text-6xl relative">
        {product.img}
      ...
      <div className="p-3 flex flex-col flex-1">
        ...
        {product.colors.length > 1 && (
          <div className="flex gap-1 mt-1">
            {product.colors.slice(0, 4).map((c) => <span key={c.name} className="w-3 h-3 rounded-full border border-gray-200" style={{ backgroundColor: c.hex }} />)}
          </div>
        )}
...
const BeautyProductDetail = () => {
  ...
  const product = beautyProducts.find((p) => p._id === id);
  ...
  if (!product) return (
    <div className="min-h-[60vh] flex flex-col items-center justify-center gap-4">
      ...
      <Link to="/beauty" className="text-primary-600 font-semibold hover:underline">&#8592; Back to Beauty</Link>
    </div>
  );
  ...
  const related  = beautyProducts.filter((p) => p._id !== product._id && p.tag === product.tag).slice(0, 6);
  const topPicks = beautyProducts.filter((p) => p._id !== product._id && p.rating >= 4.4).slice(0, 6);
    ...
    <div className="bg-gray-50 min-h-screen">
      <div className="max-w-[1280px] mx-auto px-4 py-3">
        <nav className="flex items-center gap-1.5 text-xs text-gray-500">
          ...
          <Link to="/beauty" className="hover:text-primary-600">Beauty</Link><span>/</span>
      ...
      <div className="max-w-[1280px] mx-auto px-4 pb-6">
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="lg:w-[42%] flex-shrink-0">
            <div className="bg-white rounded-2xl border border-gray-100 p-4 sticky top-24">
              <div className="relative h-80 bg-gradient-to-br from-pink-50 to-rose-50 rounded-xl flex items-center justify-center text-[120px] mb-3 overflow-hidden">
              ...
              <div className="flex gap-2 overflow-x-auto pb-1">
                {product.images.map((img, i) => (
                  <button key={i} onClick={() => setImgIdx(i)}
                    className={`w-14 h-14 flex-shrink-0 rounded-xl bg-gradient-to-br from-pink-50 to-rose-50 flex items-center justify-center text-3xl border-2 transition-all ${imgIdx === i ? "border-primary-500 shadow-sm" : "border-transparent hover:border-gray-300"}`}>
                    {img}
                  </button>
                ))}
              </div>
          ...
          <div className="flex-1 min-w-0 space-y-4">
            ...
            {/* Shade / Color */}
            {product.colors.length > 1 && product.colors[0]?.name !== "N/A" && (
              <div className="bg-white rounded-2xl border border-gray-100 p-5">
                <p className="text-sm font-bold text-gray-800 mb-3">Shade: <span className="text-primary-600">{selectedColor}</span></p>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-5">
              ...
              <div className="flex gap-3">
                <button onClick={handleAddToCart} className="flex-1 flex items-center justify-center gap-2 bg-pink-500 hover:bg-pink-600 text-white font-bold py-3.5 rounded-xl transition-all shadow-md shadow-pink-200 active:scale-[0.98]">
              ...
              <div className="mt-4 grid grid-cols-3 gap-3">
                {[
                  ...
                  { icon: <MdLoop size={20} className="text-green-600" />,           label: "Easy Returns",  sub: "7-day return" },
                  { icon: <MdSecurity size={20} className="text-blue-600" />,        label: "100% Authentic",sub: "Verified seller" },
                ].map((item) => (
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-4 flex items-center justify-between">
              ...
              <span className="text-xs text-green-600 font-semibold bg-green-50 border border-green-200 px-2 py-1 rounded-lg">Authentic Products</span>
            </div>
          </div>
        </div>
        ...
        <div className="mt-6 bg-white rounded-2xl border border-gray-100 overflow-hidden">
          <div className="flex border-b border-gray-100">
            {[{ key: "desc", label: "Description" }, { key: "highlights", label: "Highlights" }, { key: "specs", label: "Details" }].map((t) => (
        ...
        {related.length > 0 && (
          <div className="mt-8">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link to="/beauty" className="text-sm text-primary-600 font-semibold hover:underline">View All</Link>
            </div>
        ...
        {topPicks.length > 0 && (
          <div className="mt-8">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-bold text-gray-900">&#11088; Top Rated Beauty</h2>
              <Link to="/beauty" className="text-sm text-primary-600 font-semibold hover:underline">View All</Link>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
