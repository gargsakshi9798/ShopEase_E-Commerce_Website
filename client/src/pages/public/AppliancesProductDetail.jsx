...
const SuggestedCard = ({ product }) => {
  ...
  return (
    <Link to={`/appliances/product/${product._id}`}
      ...
      <div className="h-36 bg-gradient-to-br from-blue-50 to-cyan-50 flex items-center justify-center text-6xl relative">
        {product.img}
...
const AppliancesProductDetail = () => {
  ...
  const product = appliancesProducts.find((p) => p._id === id);
  ...
  if (!product) return (
    <div className="min-h-[60vh] flex flex-col items-center justify-center gap-4">
      ...
      <Link to="/appliances" className="text-primary-600 font-semibold hover:underline">&#8592; Back to Appliances</Link>
    </div>
  );
  ...
  const related  = appliancesProducts.filter((p) => p._id !== product._id && p.tag === product.tag).slice(0, 6);
  const topPicks = appliancesProducts.filter((p) => p._id !== product._id && p.rating >= 4.4).slice(0, 6);

  const handleAddToCart = () => { dispatch(addToCart({ product: { ...product, selectedColor }, qty })); toast.success(`${product.name} added to cart!`); };
  const handleBuyNow    = () => { dispatch(addToCart({ product: { ...product, selectedColor }, qty })); navigate("/checkout"); };
  const applyCoupon     = () => {
    ...
    <div className="bg-gray-50 min-h-screen">
      <div className="max-w-[1280px] mx-auto px-4 py-3">
        <nav className="flex items-center gap-1.5 text-xs text-gray-500">
          ...
          <Link to="/appliances" className="hover:text-primary-600">Appliances</Link><span>/</span>
      ...
      <div className="max-w-[1280px] mx-auto px-4 pb-6">
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="lg:w-[42%] flex-shrink-0">
            <div className="bg-white rounded-2xl border border-gray-100 p-4 sticky top-24">
              <div className="relative h-80 bg-gradient-to-br from-blue-50 to-cyan-50 rounded-xl flex items-center justify-center text-[120px] mb-3 overflow-hidden">
              ...
              <div className="flex gap-2 overflow-x-auto pb-1">
                {product.images.map((img, i) => (
                  <button key={i} onClick={() => setImgIdx(i)}
                    className={`w-14 h-14 flex-shrink-0 rounded-xl bg-gradient-to-br from-blue-50 to-cyan-50 flex items-center justify-center text-3xl border-2 transition-all ${imgIdx === i ? "border-primary-500 shadow-sm" : "border-transparent hover:border-gray-300"}`}>
                    {img}
                  </button>
                ))}
              </div>
          ...
          <div className="flex-1 min-w-0 space-y-4">
            <div className="bg-white rounded-2xl border border-gray-100 p-5">
              ...
              <div className="mt-4 pb-4 border-b border-gray-100">
                ...
                <p className="text-xs text-gray-500 mt-1">Inclusive of all taxes. Free installation available.</p>
              </div>
              <div className="mt-4">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-5">
              ...
              <div className="flex gap-3">
                <button onClick={handleAddToCart} className="flex-1 flex items-center justify-center gap-2 bg-amber-500 hover:bg-amber-600 text-white font-bold py-3.5 rounded-xl transition-all shadow-md shadow-amber-200 active:scale-[0.98]">
              ...
              <div className="mt-4 grid grid-cols-3 gap-3">
                {[
                  ...
                  { icon: <MdLoop size={20} className="text-green-600" />,            label: "Free Installation", sub: "By certified tech" },
                  { icon: <MdSecurity size={20} className="text-blue-600" />,         label: "Warranty",          sub: "Brand warranty" },
                ].map((item) => (
        ...
        {related.length > 0 && (
          <div className="mt-8">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-bold text-gray-900">&#9889; Similar Appliances</h2>
              <Link to="/appliances" className="text-sm text-primary-600 font-semibold hover:underline">View All</Link>
            </div>
        ...
        {topPicks.length > 0 && (
          <div className="mt-8">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-bold text-gray-900">&#11088; Top Rated Appliances</h2>
              <Link to="/appliances" className="text-sm text-primary-600 font-semibold hover:underline">View All</Link>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
