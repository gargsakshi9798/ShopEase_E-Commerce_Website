...
import { automotiveProducts, discount, badgeColor } from "../../data/automotiveData";
...
const SuggestedCard = ({ product }) => {
  ...
  return (
    <Link to={`/automotive/product/${product._id}`}
      className="bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-md transition-shadow flex flex-col">
      <div className="h-36 bg-gradient-to-br from-slate-50 to-zinc-50 flex items-center justify-center text-6xl relative">
        {product.img}
      ...
      <div className="p-3 flex flex-col flex-1">
        ...
        <div className="flex items-center gap-1 mt-1">
          <div className="flex items-center gap-0.5 bg-green-600 text-white text-[10px] px-1 py-0.5 rounded font-bold">{product.rating} <MdStar size={9} /></div>
        </div>
...
const AutomotiveProductDetail = () => {
  const { id }   = useParams();
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const wishlist = useSelector((s) => s.publicWishlist.items);
  const product  = automotiveProducts.find((p) => p._id === id);
  ...
  if (!product) return (
    <div className="min-h-[60vh] flex flex-col items-center justify-center gap-4">
      ...
      <Link to="/automotive" className="text-primary-600 font-semibold hover:underline">&#8592; Back to Automotive</Link>
    </div>
  );
  ...
  const related  = automotiveProducts.filter((p) => p._id !== product._id && p.tag === product.tag).slice(0, 6);
  const topPicks = automotiveProducts.filter((p) => p._id !== product._id && p.rating >= 4.4).slice(0, 6);
    ...
    <div className="bg-gray-50 min-h-screen">
      ...
      <div className="max-w-[1280px] mx-auto px-4 py-3">
        <nav className="flex items-center gap-1.5 text-xs text-gray-500">
          ...
          <Link to="/automotive" className="hover:text-primary-600">Automotive</Link><span>/</span>
      ...
      <div className="max-w-[1280px] mx-auto px-4 pb-6">
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="lg:w-[42%] flex-shrink-0">
            <div className="bg-white rounded-2xl border border-gray-100 p-4 sticky top-24">
              <div className="relative h-80 bg-gradient-to-br from-slate-50 to-zinc-50 rounded-xl flex items-center justify-center text-[120px] mb-3 overflow-hidden">
                ...
                {imgIdx > 0 && (
                  <button onClick={() => setImgIdx((i) => i - 1)} className="absolute left-2 top-1/2 -translate-y-1/2 w-8 h-8 bg-white/80 rounded-full flex items-center justify-center shadow hover:bg-white">
                    <MdChevronLeft size={20} />
                  </button>
                )}
                {imgIdx < product.images.length - 1 && (
                  <button onClick={() => setImgIdx((i) => i + 1)} className="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 bg-white/80 rounded-full flex items-center justify-center shadow hover:bg-white">
              ...
              <div className="flex gap-2 overflow-x-auto pb-1">
                {product.images.map((img, i) => (
                  <button key={i} onClick={() => setImgIdx(i)}
                    className={`w-14 h-14 flex-shrink-0 rounded-xl bg-gradient-to-br from-slate-50 to-zinc-50 flex items-center justify-center text-3xl border-2 transition-all ${imgIdx === i ? "border-primary-500 shadow-sm" : "border-transparent hover:border-gray-300"}`}>
                    {img}
                  </button>
                ))}
              </div>
          ...
          <div className="flex-1 min-w-0 space-y-4">
            ...
            {product.sizes.length > 1 && product.sizes[0] !== "One Size" && (
              <div className="bg-white rounded-2xl border border-gray-100 p-5">
                <p className="text-sm font-bold text-gray-800 mb-3">Size / Variant: <span className="text-primary-600">{selectedSize}</span></p>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-5">
              ...
              <div className="flex gap-3">
                <button onClick={handleAddToCart}
                  className="flex-1 flex items-center justify-center gap-2 bg-red-500 hover:bg-red-600 text-white font-bold py-3.5 rounded-xl transition-all shadow-md shadow-red-200 active:scale-[0.98]">
              ...
              <div className="mt-4 grid grid-cols-3 gap-3">
                {[
                  { icon: <MdLocalShipping size={20} className="text-primary-600" />, label: "Free Delivery",   sub: `in ${product.deliveryDays} days` },
                  { icon: <MdLoop size={20} className="text-green-600" />,            label: "Expert Support",  sub: "Installation help" },
                  { icon: <MdSecurity size={20} className="text-blue-600" />,         label: "Genuine Parts",   sub: "100% Authentic" },
                ].map((item) => (
        ...
        {/* Description / Highlights / Specs Tabs */}
        ...
        {related.length > 0 && (
          <div className="mt-8">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link to="/automotive" className="text-sm text-primary-600 font-semibold hover:underline">View All</Link>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
              {related.map((p) => <SuggestedCard key={p._id} product={p} />)}
            </div>
          </div>
        )}
        ...
        {topPicks.length > 0 && (
          <div className="mt-8">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-bold text-gray-900">&#11088; Top Rated Auto Parts</h2>
              <Link to="/automotive" className="text-sm text-primary-600 font-semibold hover:underline">View All</Link>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
              {topPicks.map((p) => <SuggestedCard key={p._id} product={p} />)}
            </div>
          </div>
        )}
      </div>
