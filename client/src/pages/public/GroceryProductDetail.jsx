...
const SuggestedCard = ({ product }) => {
  ...
  return (
    <Link to={`/grocery/product/${product._id}`}
      ...
      <div className="h-36 bg-gradient-to-br from-green-50 to-lime-50 flex items-center justify-center text-6xl relative">
        {product.img}
...
const GroceryProductDetail = () => {
  ...
  const product  = groceryProducts.find((p) => p._id === id);
  ...
  if (!product) return (
    <div className="min-h-[60vh] flex flex-col items-center justify-center gap-4">
      ...
      <Link to="/grocery" className="text-primary-600 font-semibold hover:underline">&#8592; Back to Grocery</Link>
    </div>
  );
  ...
  const related  = groceryProducts.filter((p) => p._id !== product._id && p.tag === product.tag).slice(0, 6);
  const topPicks = groceryProducts.filter((p) => p._id !== product._id && p.rating >= 4.4).slice(0, 6);
    ...
    <div className="bg-gray-50 min-h-screen">
      ...
      <div className="max-w-[1280px] mx-auto px-4 py-3">
        <nav className="flex items-center gap-1.5 text-xs text-gray-500">
          ...
          <Link to="/grocery" className="hover:text-primary-600">Grocery</Link><span>/</span>
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="lg:w-[42%] flex-shrink-0">
            <div className="bg-white rounded-2xl border border-gray-100 p-4 sticky top-24">
              <div className="relative h-80 bg-gradient-to-br from-green-50 to-lime-50 rounded-xl flex items-center justify-center text-[120px] mb-3 overflow-hidden">
              ...
              <div className="flex gap-2 overflow-x-auto pb-1">
                {product.images.map((img, i) => (
                  <button key={i} onClick={() => setImgIdx(i)}
                    className={`w-14 h-14 flex-shrink-0 rounded-xl bg-gradient-to-br from-green-50 to-lime-50 flex items-center justify-center text-3xl border-2 transition-all ${imgIdx === i ? "border-primary-500 shadow-sm" : "border-transparent hover:border-gray-300"}`}>
                    {img}
                  </button>
                ))}
              </div>
          ...
          <div className="flex-1 min-w-0 space-y-4">
            ...
            {product.sizes.length > 1 && product.sizes[0] !== "One Size" && (
              <div className="bg-white rounded-2xl border border-gray-100 p-5">
                <p className="text-sm font-bold text-gray-800 mb-3">Pack Size: <span className="text-primary-600">{selectedSize}</span></p>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-5">
              ...
              <div className="flex gap-3">
                <button onClick={handleAddToCart} className="flex-1 flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700 text-white font-bold py-3.5 rounded-xl transition-all shadow-md shadow-green-200 active:scale-[0.98]">
              ...
              <div className="mt-4 grid grid-cols-3 gap-3">
                {[
                  { icon: <MdLocalShipping size={20} className="text-primary-600" />, label: "Fast Delivery",     sub: `in ${product.deliveryDays} day` },
                  { icon: <MdLoop size={20} className="text-green-600" />,            label: "Fresh Products",    sub: "Quality guaranteed" },
                  { icon: <MdSecurity size={20} className="text-blue-600" />,         label: "FSSAI Certified",   sub: "Safe to consume" },
                ].map((item) => (
        ...
        {related.length > 0 && (
          <div className="mt-8">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-bold text-gray-900">&#127820; More from this Category</h2>
              <Link to="/grocery" className="text-sm text-primary-600 font-semibold hover:underline">View All</Link>
            </div>
