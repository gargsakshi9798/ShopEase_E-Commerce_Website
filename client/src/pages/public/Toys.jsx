...
const subcategories = [
  { icon: "🧱", label: "Building Blocks", tab: "building",      count: 430 },
  { icon: "🎲", label: "Board Games",     tab: "board_games",   count: 620 },
  { icon: "🚗", label: "Vehicles",        tab: "vehicles",      count: 540 },
  { icon: "👸", label: "Dolls",           tab: "dolls",         count: 380 },
  { icon: "🎨", label: "Arts & Crafts",   tab: "creative",      count: 290 },
  { icon: "🚁", label: "RC & Drones",     tab: "remote_control",count: 210 },
  { icon: "🧩", label: "Puzzles",         tab: "puzzles",       count: 340 },
  { icon: "📚", label: "Educational",     tab: "educational",   count: 680 },
];

const brands = ["LEGO", "Hot Wheels", "Barbie", "Hasbro", "Nerf", "Funskool", "Melissa & Doug", "VTech", "Rubik's", "Syma", "Dickie Toys"];
const sortOptions = [
...
const ProductCard = ({ product, view }) => {
  ...
  if (view === "list") {
    return (
      <Link to={`/toys/product/${product._id}`} className="bg-white rounded-2xl shadow-sm border border-gray-100 flex gap-4 p-4 hover:shadow-md transition-shadow group">
        <div className="w-32 h-32 bg-gradient-to-br from-pink-50 to-yellow-50 rounded-xl flex items-center justify-center text-6xl flex-shrink-0 relative">
          {product.img}
  ...
  return (
    <Link to={`/toys/product/${product._id}`} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow group flex flex-col">
      <div className="relative bg-gradient-to-br from-pink-50 to-yellow-50 h-44 flex items-center justify-center text-7xl">
        {product.img}
        ...
        <span className="absolute top-3 right-3 text-xs bg-purple-100 text-purple-700 font-bold px-1.5 py-0.5 rounded-lg">{disc}% off</span>
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-4 sticky top-24">
      ...
      <Section title="Price Range" id="price">
        <div className="px-1">
          <input type="range" min={0} max={10000} step={500} value={priceRange[1]} onChange={(e) => setPriceRange([0, +e.target.value])} className="w-full accent-primary-600" />
...
const Toys = () => {
  ...
  const tabs = [
    ...
    { key: "remote_control",label: "RC & Drones" },
    { key: "educational",  label: "Educational" },
    { key: "puzzles",      label: "Puzzles" },
  ];

  const filtered = activeTab === "all" ? toysProducts : toysProducts.filter((p) => p.tag === activeTab);
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      {/* Hero */}
      <div className="bg-gradient-to-r from-[#450a78] via-[#7e22ce] to-[#a855f7] text-white">
        <div className="max-w-[1280px] mx-auto px-4 py-10 flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            <div className="flex items-center gap-2 mb-3">
              <FaGamepad className="text-yellow-300" size={18} />
              <span className="text-yellow-300 font-bold text-sm tracking-wide uppercase">Toys &amp; Games Sale</span>
            </div>
            <h1 className="text-4xl md:text-5xl font-extrabold leading-tight mb-3">
              Toys &amp; Games for <span className="text-yellow-300">Every Age</span>
            </h1>
            <p className="text-white/70 max-w-md text-sm leading-relaxed mb-5">
              LEGO sets, board games, RC toys, dolls and more from top brands. Up to 50% off. Use coupon <span className="text-yellow-300 font-bold">TOYS15</span> for extra savings.
            </p>
            <div className="flex flex-wrap gap-3 mb-5">
              {["Free Delivery", "Safe for Kids", "Genuine Products", "EMI Available"].map((f) => (
            ...
            <div className="flex items-center gap-3">
              <Link to="/" className="inline-flex items-center gap-2 bg-yellow-400 hover:bg-yellow-500 text-purple-900 font-bold px-6 py-3 rounded-xl text-sm transition-colors shadow-lg">
              ...
              <div className="flex items-center gap-2 bg-white/10 backdrop-blur px-4 py-3 rounded-xl border border-white/20">
                ...
                <span className="text-sm font-bold">Use <span className="text-yellow-300">TOYS15</span> &#8594; Extra 15% off</span>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-4 sm:grid-cols-8 gap-3">
          {subcategories.map((s) => (
            <button key={s.label} onClick={() => setActiveTab(s.tab)}
              className="bg-white rounded-2xl p-3 flex flex-col items-center gap-2 shadow-sm border border-gray-100 hover:border-purple-300 hover:shadow-md transition-all group">
              <span className="text-3xl">{s.icon}</span>
              <span className="text-[11px] font-semibold text-gray-700 group-hover:text-purple-700 text-center leading-tight">{s.label}</span>
      ...
      <div className="max-w-[1280px] mx-auto px-4 pb-4">
        <div className="bg-gradient-to-r from-purple-50 to-pink-50 border border-purple-100 rounded-2xl p-4 flex flex-wrap items-center gap-4">
          <div className="flex items-center gap-2">
            <span className="text-2xl">🧸</span>
            <div>
              <p className="text-sm font-bold text-gray-900">Today's Toy Deals</p>
              <p className="text-xs text-gray-500">Limited time offers on top toy brands</p>
            </div>
          </div>
          {[
            { label: "Puzzles from",    price: "&#8377;349" },
            { label: "Board Games from",price: "&#8377;549" },
            { label: "RC Toys from",    price: "&#8377;1,299" },
            { label: "LEGO Sets from",  price: "&#8377;2,499" },
          ].map((d) => (
            <div key={d.label} className="bg-white rounded-xl px-3 py-2 border border-purple-100 text-center shadow-sm">
              <p className="text-[10px] text-gray-500">{d.label}</p>
              <p className="text-sm font-bold text-purple-700" dangerouslySetInnerHTML={{ __html: d.price }} />
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="bg-white rounded-2xl border border-gray-100 px-4 py-3 flex flex-wrap items-center gap-3 mb-4 shadow-sm">
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              <button onClick={() => setMobileFilter(true)} className="lg:hidden flex items-center gap-1.5 border border-gray-200 rounded-lg px-3 py-1.5 text-xs font-medium text-gray-600">
          ...
          {sorted.length === 0 ? (
            <div className="bg-white rounded-2xl border border-gray-100 p-16 text-center">
              ...
              <button onClick={() => { setActiveTab("all"); setFilters({ brands: [] }); }} className="mt-3 text-sm text-primary-600 font-semibold hover:underline">Clear filters</button>
            </div>
          ) : (
