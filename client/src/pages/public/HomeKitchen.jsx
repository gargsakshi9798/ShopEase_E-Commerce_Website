...
import { homeKitchenProducts, discount, badgeColor } from "../../data/homeKitchenData";

const subcategories = [
  { icon: "🍳", label: "Cookware",      tab: "cookware",   count: 840 },
  { icon: "🫙", label: "Storage",       tab: "storage",    count: 620 },
  { icon: "🍽️", label: "Dining",        tab: "dining",     count: 530 },
  { icon: "🪑", label: "Furniture",     tab: "furniture",  count: 390 },
  { icon: "🥤", label: "Kitchen Appl.", tab: "kitchen",    count: 710 },
  { icon: "🧹", label: "Cleaning",      tab: "all",        count: 480 },
  { icon: "🛏️", label: "Bedding",       tab: "all",        count: 340 },
  { icon: "🪴", label: "Decor",         tab: "all",        count: 290 },
];

const brands = ["Prestige", "Milton", "Pigeon", "Wonderchef", "Borosil", "Cello", "IKEA", "Nilkamal", "Philips", "Amazon Basics"];
...
const ProductCard = ({ product, view }) => {
  ...
  if (view === "list") {
    return (
      <Link to={`/home-kitchen/product/${product._id}`}
        className="bg-white rounded-2xl shadow-sm border border-gray-100 flex gap-4 p-4 hover:shadow-md transition-shadow group">
        <div className="w-32 h-32 bg-gradient-to-br from-orange-50 to-amber-50 rounded-xl flex items-center justify-center text-6xl flex-shrink-0 relative">
          {product.img}
  ...
  return (
    <Link to={`/home-kitchen/product/${product._id}`}
      ...
      <div className="relative bg-gradient-to-br from-orange-50 to-amber-50 h-44 flex items-center justify-center text-7xl">
        {product.img}
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-4 sticky top-24">
      ...
      <Section title="Price Range" id="price">
        <div className="px-1">
          <input type="range" min={0} max={15000} step={500} value={priceRange[1]} onChange={(e) => setPriceRange([0, +e.target.value])} className="w-full accent-primary-600" />
...
const HomeKitchen = () => {
  ...
  const tabs = [
    ...
    { key: "kitchen",  label: "Kitchen Appl." },
  ];

  const filtered = activeTab === "all" ? homeKitchenProducts : homeKitchenProducts.filter((p) => p.tag === activeTab);
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      {/* Hero */}
      <div className="bg-gradient-to-r from-[#7c3a00] via-[#b45309] to-[#d97706] text-white">
        <div className="max-w-[1280px] mx-auto px-4 py-10 flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            <div className="flex items-center gap-2 mb-3">
              <FaHome className="text-amber-300" size={18} />
              <span className="text-amber-300 font-bold text-sm tracking-wide uppercase">Home Fest Sale</span>
            </div>
            <h1 className="text-4xl md:text-5xl font-extrabold leading-tight mb-3">
              Make Your Home <span className="text-amber-300">Beautiful</span>
            </h1>
            <p className="text-white/70 max-w-md text-sm leading-relaxed mb-5">
              Discover premium cookware, stylish dining sets, smart storage and home furniture from India's most trusted brands. Up to 60% off.
            </p>
            <div className="flex flex-wrap gap-3 mb-5">
              {["Free Delivery", "Easy Returns", "Trusted Brands", "EMI Available"].map((f) => (
            ...
            <div className="flex items-center gap-3">
              <Link to="/" className="inline-flex items-center gap-2 bg-amber-400 hover:bg-amber-500 text-amber-900 font-bold px-6 py-3 rounded-xl text-sm transition-colors shadow-lg">
              ...
              <div className="flex items-center gap-2 bg-white/10 backdrop-blur px-4 py-3 rounded-xl border border-white/20">
                ...
                <span className="text-sm font-bold">Use <span className="text-yellow-300">HOME20</span> &#8594; Extra 20% off</span>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-4 sm:grid-cols-8 gap-3">
          {subcategories.map((s) => (
            <button key={s.label} onClick={() => setActiveTab(s.tab)}
              className="bg-white rounded-2xl p-3 flex flex-col items-center gap-2 shadow-sm border border-gray-100 hover:border-amber-300 hover:shadow-md transition-all group">
              <span className="text-3xl">{s.icon}</span>
              <span className="text-[11px] font-semibold text-gray-700 group-hover:text-amber-700 text-center leading-tight">{s.label}</span>
      ...
      <div className="max-w-[1280px] mx-auto px-4 pb-4">
        <div className="bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-100 rounded-2xl p-4 flex flex-wrap items-center gap-4">
          <div className="flex items-center gap-2">
            <span className="text-2xl">🏠</span>
            <div>
              <p className="text-sm font-bold text-gray-900">Today's Home Deals</p>
              <p className="text-xs text-gray-500">Limited time offers on top home brands</p>
            </div>
          </div>
          {[{ label: "Cookware from", price: "&#8377;299" }, { label: "Storage from", price: "&#8377;449" }, { label: "Dining Sets from", price: "&#8377;1,899" }, { label: "Furniture from", price: "&#8377;3,299" }].map((d) => (
            <div key={d.label} className="bg-white rounded-xl px-3 py-2 border border-amber-100 text-center shadow-sm">
              <p className="text-[10px] text-gray-500">{d.label}</p>
              <p className="text-sm font-bold text-amber-700" dangerouslySetInnerHTML={{ __html: d.price }} />
            </div>
          ))}
        </div>
      </div>
