...
import { appliancesProducts, discount, badgeColor } from "../../data/appliancesData";

const subcategories = [
  { icon: "🧊", label: "Refrigerators",  tab: "refrigerator",    count: 420 },
  { icon: "🫧", label: "Washing Machines",tab: "washing_machine", count: 380 },
  { icon: "❄️", label: "Air Conditioners",tab: "ac",              count: 290 },
  { icon: "📻", label: "Microwaves",      tab: "microwave",       count: 240 },
  { icon: "🌀", label: "Fans",            tab: "fan",             count: 560 },
  { icon: "🔥", label: "Heaters",         tab: "heater",          count: 180 },
  { icon: "🌬️", label: "Air Purifiers",   tab: "air_purifier",    count: 150 },
  { icon: "💧", label: "Water Purifiers", tab: "water_purifier",  count: 210 },
];

const brands = ["LG", "Samsung", "Voltas", "Bajaj", "Butterfly", "Havells", "Whirlpool", "Usha", "Crompton", "Atomberg", "Philips", "Eureka Forbes"];
...
const ProductCard = ({ product, view }) => {
  ...
  if (view === "list") {
    return (
      <Link to={`/appliances/product/${product._id}`}
        className="bg-white rounded-2xl shadow-sm border border-gray-100 flex gap-4 p-4 hover:shadow-md transition-shadow group">
        <div className="w-32 h-32 bg-gradient-to-br from-blue-50 to-cyan-50 rounded-xl flex items-center justify-center text-6xl flex-shrink-0 relative">
          {product.img}
  ...
  return (
    <Link to={`/appliances/product/${product._id}`}
      ...
      <div className="relative bg-gradient-to-br from-blue-50 to-cyan-50 h-44 flex items-center justify-center text-7xl">
        {product.img}
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-4 sticky top-24">
      ...
      <Section title="Price Range" id="price">
        <div className="px-1">
          <input type="range" min={0} max={50000} step={1000} value={priceRange[1]} onChange={(e) => setPriceRange([0, +e.target.value])} className="w-full accent-primary-600" />
...
const Appliances = () => {
  ...
  const tabs = [
    ...
    { key: "washing_machine", label: "Washers" },
    { key: "ac",              label: "ACs" },
    { key: "fan",             label: "Fans" },
    { key: "microwave",       label: "Microwaves" },
  ];

  const filtered = activeTab === "all" ? appliancesProducts : appliancesProducts.filter((p) => p.tag === activeTab);
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      {/* Hero */}
      <div className="bg-gradient-to-r from-[#0c1445] via-[#1a2980] to-[#26d0ce] text-white">
        <div className="max-w-[1280px] mx-auto px-4 py-10 flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            <div className="flex items-center gap-2 mb-3">
              <FaBolt className="text-cyan-300" size={18} />
              <span className="text-cyan-300 font-bold text-sm tracking-wide uppercase">Appliances Mega Sale</span>
            </div>
            <h1 className="text-4xl md:text-5xl font-extrabold leading-tight mb-3">
              Power Up Your <span className="text-cyan-300">Home</span>
            </h1>
            <p className="text-white/70 max-w-md text-sm leading-relaxed mb-5">
              Top-rated ACs, refrigerators, washing machines, fans and more from India's most trusted brands. Up to 50% off + no-cost EMI.
            </p>
            <div className="flex flex-wrap gap-3 mb-5">
              {["No-Cost EMI", "Free Installation", "1-Year Warranty", "Free Delivery"].map((f) => (
            ...
            <div className="flex items-center gap-3">
              <Link to="/" className="inline-flex items-center gap-2 bg-cyan-400 hover:bg-cyan-500 text-cyan-900 font-bold px-6 py-3 rounded-xl text-sm transition-colors shadow-lg">
              ...
              <div className="flex items-center gap-2 bg-white/10 backdrop-blur px-4 py-3 rounded-xl border border-white/20">
                ...
                <span className="text-sm font-bold">Use <span className="text-yellow-300">APPLIA15</span> &#8594; 15% off</span>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-4 sm:grid-cols-8 gap-3">
          {subcategories.map((s) => (
            <button key={s.label} onClick={() => setActiveTab(s.tab)}
              ...
              <span className="text-[11px] font-semibold text-gray-700 group-hover:text-cyan-700 text-center leading-tight">{s.label}</span>
      ...
      <div className="max-w-[1280px] mx-auto px-4 pb-4">
        <div className="bg-gradient-to-r from-blue-50 to-cyan-50 border border-blue-100 rounded-2xl p-4 flex flex-wrap items-center gap-4">
          <div className="flex items-center gap-2">
            <span className="text-2xl">⚡</span>
            <div>
              <p className="text-sm font-bold text-gray-900">Today's Appliance Deals</p>
              <p className="text-xs text-gray-500">Limited time offers with free installation</p>
            </div>
          </div>
          {[{ label: "Fans from", price: "&#8377;1,499" }, { label: "Irons from", price: "&#8377;799" }, { label: "Microwaves from", price: "&#8377;5,999" }, { label: "Fridges from", price: "&#8377;16,999" }].map((d) => (
            <div key={d.label} className="bg-white rounded-xl px-3 py-2 border border-blue-100 text-center shadow-sm">
              <p className="text-[10px] text-gray-500">{d.label}</p>
              <p className="text-sm font-bold text-blue-700" dangerouslySetInnerHTML={{ __html: d.price }} />
            </div>
          ))}
        </div>
      </div>
