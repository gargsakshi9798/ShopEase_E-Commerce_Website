...
import { sportsProducts, discount, badgeColor } from "../../data/sportsData";

const subcategories = [
  { icon: "⚽", label: "Football",    tab: "outdoor",     count: 840 },
  { icon: "🏏", label: "Cricket",     tab: "cricket",     count: 620 },
  { icon: "🏸", label: "Badminton",   tab: "racket",      count: 430 },
  { icon: "🏋️", label: "Gym & Fitness",tab:"fitness",     count: 980 },
  { icon: "🏊", label: "Swimming",    tab: "swimming",    count: 210 },
  { icon: "👟", label: "Sports Shoes",tab: "footwear",    count: 760 },
  { icon: "🧘", label: "Yoga",        tab: "fitness",     count: 390 },
  { icon: "👕", label: "Sportswear",  tab: "sportswear",  count: 540 },
];

const brands = ["Adidas", "Nike", "Puma", "Yonex", "Cosco", "Nivia", "SG", "Speedo", "Decathlon", "Boldfit", "Fitkit", "Strauss"];
const sortOptions = [
...
const ProductCard = ({ product, view }) => {
  ...
  if (view === "list") {
    return (
      <Link to={`/sports/product/${product._id}`} className="bg-white rounded-2xl shadow-sm border border-gray-100 flex gap-4 p-4 hover:shadow-md transition-shadow group">
        <div className="w-32 h-32 bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl flex items-center justify-center text-6xl flex-shrink-0 relative">
          {product.img}
  ...
  return (
    <Link to={`/sports/product/${product._id}`} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow group flex flex-col">
      <div className="relative bg-gradient-to-br from-green-50 to-emerald-50 h-44 flex items-center justify-center text-7xl">
        {product.img}
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-4 sticky top-24">
      ...
      <Section title="Price Range" id="price">
        <div className="px-1">
          <input type="range" min={0} max={35000} step={500} value={priceRange[1]} onChange={(e) => setPriceRange([0, +e.target.value])} className="w-full accent-primary-600" />
...
const Sports = () => {
  ...
  const filtered = activeTab === "all" ? sportsProducts : sportsProducts.filter((p) => p.tag === activeTab);
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      {/* Hero */}
      <div className="bg-gradient-to-r from-[#052e16] via-[#166534] to-[#16a34a] text-white">
        <div className="max-w-[1280px] mx-auto px-4 py-10 flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            <div className="flex items-center gap-2 mb-3">
              <FaRunning className="text-lime-300" size={18} />
              <span className="text-lime-300 font-bold text-sm tracking-wide uppercase">Sports Mega Sale</span>
            </div>
            <h1 className="text-4xl md:text-5xl font-extrabold leading-tight mb-3">
              Play Hard, <span className="text-lime-300">Train Harder</span>
            </h1>
            <p className="text-white/70 max-w-md text-sm leading-relaxed mb-5">
              Top cricket gear, fitness equipment, outdoor sports essentials and premium sportswear from India's favourite brands. Up to 55% off.
            </p>
            ...
            <div className="flex items-center gap-3">
              <Link to="/" className="inline-flex items-center gap-2 bg-lime-400 hover:bg-lime-500 text-green-900 font-bold px-6 py-3 rounded-xl text-sm transition-colors shadow-lg">
              ...
              <div className="flex items-center gap-2 bg-white/10 backdrop-blur px-4 py-3 rounded-xl border border-white/20">
                ...
                <span className="text-sm font-bold">Use <span className="text-yellow-300">SPORT20</span> &#8594; Extra 20% off</span>
              </div>
            </div>
          </div>
      ...
      <div className="max-w-[1280px] mx-auto px-4 py-6">
        <h2 className="text-lg font-bold text-gray-900 mb-4">Shop by Sport</h2>
        <div className="grid grid-cols-4 sm:grid-cols-8 gap-3">
          {subcategories.map((s) => (
            <button key={s.label} onClick={() => setActiveTab(s.tab)}
              className="bg-white rounded-2xl p-3 flex flex-col items-center gap-2 shadow-sm border border-gray-100 hover:border-green-300 hover:shadow-md transition-all group">
              <span className="text-3xl">{s.icon}</span>
              <span className="text-[11px] font-semibold text-gray-700 group-hover:text-green-700 text-center leading-tight">{s.label}</span>
      ...
      <div className="max-w-[1280px] mx-auto px-4 pb-4">
        <div className="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-100 rounded-2xl p-4 flex flex-wrap items-center gap-4">
          <div className="flex items-center gap-2">
            <span className="text-2xl">🏆</span>
            <div>
              <p className="text-sm font-bold text-gray-900">Today's Sports Deals</p>
              <p className="text-xs text-gray-500">Limited time offers on top sports brands</p>
            </div>
          </div>
          {[{ label: "Fitness from", price: "&#8377;299" }, { label: "Cricket Gear from", price: "&#8377;699" }, { label: "Rackets from", price: "&#8377;1,299" }, { label: "Treadmills from", price: "&#8377;24,999" }].map((d) => (
            <div key={d.label} className="bg-white rounded-xl px-3 py-2 border border-green-100 text-center shadow-sm">
              <p className="text-[10px] text-gray-500">{d.label}</p>
              <p className="text-sm font-bold text-green-700" dangerouslySetInnerHTML={{ __html: d.price }} />
            </div>
          ))}
        </div>
      </div>
